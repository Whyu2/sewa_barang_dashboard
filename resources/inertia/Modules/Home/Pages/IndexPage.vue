...
<template>
    <DashboardLayout>
        ...
        <div v-if="chartsReady" class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-surface-0 dark:bg-surface-900 border border-surface-200 dark:border-surface-800 shadow rounded p-4"><p class="font-semibold mb-2 text-surface-900 dark:text-surface-0">Pendapatan per Wilayah</p><Chart type="bar" :data="regionData" :options="columnOptions" /></div>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div class="bg-surface-0 dark:bg-surface-900 border border-surface-200 dark:border-surface-800 shadow rounded p-4">
                ...
                <DataTable :value="tables?.recent || []" size="small"><Column header="ID Transaksi" style="width:6rem"><template #body="{data}"><span class="font-mono text-xs">{{ formatTRX(data.id) }}</span></template></Column><Column header="Produk"><template #body="{data}">{{ data.product?.name }}</template></Column><Column header="Penyewa" field="renter_name"/><Column header="Tanggal Sewa"><template #body="{data}">{{ formatDateID(data.rent_date) }}</template></Column><Column header="Status"><template #body="{data}"><Tag :value="getStatusLabel(data.status)" :severity="getStatusSeverity(data.status)"/></template></Column></DataTable>
            </div>
            <div class="bg-surface-0 dark:bg-surface-900 border border-surface-200 dark:border-surface-800 shadow rounded p-4">
                <p class="font-semibold mb-2 text-surface-900 dark:text-surface-0">Pengembalian Telat ({{ tables?.overdue?.length || 0 }})</p>
                <DataTable :value="tables?.overdue || []" size="small"><Column header="ID Transaksi" style="width:6rem"><template #body="{data}"><span class="font-mono text-xs">{{ formatTRX(data.id) }}</span></template></Column><Column header="Produk"><template #body="{data}">{{ data.product?.name }}</template></Column><Column header="Perkiraan Kembali"><template #body="{data}">{{ formatDateID(data.expected_return_date) }}</template></Column><Column header="Status"><template #body="{data}"><Tag :value="getStatusLabel(data.status)" :severity="getStatusSeverity(data.status)"/></template></Column></DataTable>
            </div>
        </div>
    </DashboardLayout>
</template>
